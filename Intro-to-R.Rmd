...
`}`
  ...
  * wilcox.test()
    ...
    `new_species <- wilcox.test(bodyLength~species, data=grasshopper, alternative=c("two.sided"), mu=3.4)`
        + here, we are asking if bodyLength is affected by species, using our grasshopper data. Alternative is two.sided because the new grasshopper body length could technically be larger or smaller than the normal grasshopper length. Mu is 3.4, because if there is no difference between the groups, then the average body length of the new grasshopper species will equal the average body length of the old grasshopper species.
