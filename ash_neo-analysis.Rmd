...
For our "simple" stats, we'll be using ANOVA and Levene's tests. ANOVAs examine differences in the mean between three+ groups, so either the mean for trial 1, 2, and 3 for amazons in one treatment, or the mean of trial 1 for amazons in all four treatments. This will tell us if the average neophobia of one trial or treatment was different than the other. Levene's test examines the variation of three+ groups. For example, if there is a lot of variation in trial 1 but very little variation in trial 2 and 3, the Levene's test can tell us if this is statistically significant (so if they habituated for weird same, or if one predator type was more scary than the others). Additionally, variation is an important question when looking at our asexual amazons: in theory, they should not have a lot of variation since they are all clones. Any variation we see in them is important and interesting!

For these analyses, you will use: aov(), summary(), car package, leveneTest()

```{r}
#is the mean for trials 1-3 different in control trials for amazons?

#how about for sailfin?


#is the mean for trials 1-3 different in weird.same trials for amazons?

#how about for sailfin?


#is the mean for trials 1-3 different in weird.diff trials for amazons?

#how about for sailfin?


#is the mean for trials 1-3 different in predator trials for amazons?

#how about for sailfin?


#is the mean for trials 1 in all 4 treatments different for amazons?

#how about for sailfin?


#is the mean for trials 2 in all 4 treatments different for amazons?

#how about for sailfin?


#is the mean for trials 3 in all 4 treatments different for amazons?

#how about for sailfin?



#repeat the combos above for variation
...
While it is nice to explore our data in the small chunks, to really address our questions we need to take a look at the whole picture. For this we need more complex statistical analyses--in our case, this is generalized linear mixed effects models (glmm).

Mixed effects models come in two flavors: linear mixed effects and generalized linear mixed effects. In essence, linear models are used on "normal" data while generalized models can take into account small samples or non-normal distributions. 
...
#perform a mixed model on the time across trial and treatments for amazons


#repeat for sailfins


#perform a mixed model on the time across treatments and species (trial as random)
